...
| Settings UI | Command palette for every toggle. v0.4 adds a **diagnostics** sidebar (read-only checkboxes; live workspace + snapshot logs). Writable settings still need host APIs — [#10](https://github.com/jondmarien/orca-discord-presence/issues/10) |
...
- **Discord Presence** sidebar panel — live workspace, Show Status toast, read-only field toggles, collapsible Extension Logs (writable installs). Palette still owns Enable / Cycle Detail / Reload RPC.
...
v0.4 ships a diagnostics sidebar, but panels cannot call `settings.set` yet — each toggle is still a command. `bridgeUrl` / `bridgeToken` persist via `settings:own` or env overlays.
...
The Application ID is public data (it is in every presence payload). Changing it requires a rebuild of `dist/`. v0.4 has no user-facing override UI (the panel never shows the id).
...
- **Diagnostics panel shipped (v0.4).** Live workspace + notifications work. Field toggles are read-only until host B4. Logs are a worker-embedded snapshot on writable installs. Host gap: [#10](https://github.com/jondmarien/orca-discord-presence/issues/10).
- A **settings** panel (writable toggles, Application ID override) is still blocked on panel-callable `settings.*` / `storage.*` ([#10](https://github.com/jondmarien/orca-discord-presence/issues/10)).
- Presence starts on the first event, command, or heartbeat — not at bare launch.
- Activity expiry helpers in [`src/presence/expiry.ts`](src/presence/expiry.ts) are for future focus/tool providers ([#7](https://github.com/jondmarien/orca-discord-presence/issues/7)) — not applied to today’s snapshot.
